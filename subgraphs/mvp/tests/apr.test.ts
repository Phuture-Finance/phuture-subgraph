...
test("Register fCash events", () => {
    ...
    createMockedFunction(frpVaultAddr, 'totalAssets', 'totalAssets():(uint256)')
        .returns([ethereum.Value.fromSignedBigInt(BigInt.fromI64(1000000000))]);
    createMockedFunction(frpVaultAddr, 'totalSupply', 'totalSupply():(uint256)')
        .returns([ethereum.Value.fromSignedBigInt(BigInt.fromI64(1000000000000000000000))]);
    createMockedFunction(frpVaultAddr, 'symbol', 'symbol():(string)')
        .returns([ethereum.Value.fromString("USDC_VAULT")]);
    createMockedFunction(frpVaultAddr, 'decimals', 'decimals():(uint8)')
        .returns([ethereum.Value.fromI32(18)]);
    createMockedFunction(frpVaultAddr, 'name', 'name():(string)')
        .returns([ethereum.Value.fromString("USDC Notional Vault Mock")]);

    eventList.forEach((event: FCashEvent) => {
        let frpVaultAddr = Address.fromString("0xf89aa2f1397e9a0622c8fc99ab1947e28b5ef876");

        // FCashBase address mock function to receive maturity.
        createMockedFunction(event.position, 'getMaturity', 'getMaturity():(uint40)')
    ...
    assert.fieldEquals(
        ...
        "apr", "0.01599301177669391899135591347481267");
