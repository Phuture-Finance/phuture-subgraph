...
test('Register base assets participated in indexes', () => {
  assets.forEach((asset: AssetData) => {
    ...
    updateAssetEvent.parameters.push(new ethereum.EventParam('asset', ethereum.Value.fromAddress(asset.address)));
    ...
    createMockedFunction(chAggAddr, 'latestAnswer', 'latestAnswer():(int256)').returns([
      ethereum.Value.fromSignedBigInt(BigInt.fromI32(500000000)),
    ]);
    createMockedFunction(chAggAddr, 'decimals', 'decimals():(uint8)').returns([ethereum.Value.fromI32(8)]);
    createMockedFunction(chAggAddr, 'description', 'description():(string)').returns([
      ethereum.Value.fromString(asset.symbol.concat(' / USD')),
    ]);
    createMockedFunction(chAggAddr, 'aggregator', 'aggregator():(address)').returns([
      ethereum.Value.fromAddress(chAggAddr),
    ]);
    ...
    createMockedFunction(asset.address, 'name', 'name():(string)').returns([ethereum.Value.fromString(asset.name)]);
    createMockedFunction(asset.address, 'totalSupply', 'totalSupply():(uint256)').returns([
      ethereum.Value.fromUnsignedBigInt(BigInt.fromString('100000000000')),
    ]);
    createMockedFunction(asset.address, 'decimals', 'decimals():(uint8)').returns([
      ethereum.Value.fromI32(asset.decimals),
    ]);
