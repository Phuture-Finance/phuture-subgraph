...
export function handleTransfer(event: Transfer): void {
  ...
  if (fromVC && toVT) {
      ...
      let ln = BigDecimal.fromString(
        Math.log(
          parseFloat(
            fromVC.withdraw
              .toBigDecimal()
              .div(fromVC.deposit.toBigDecimal())
              .toString(),
          ),
        ).toString(),
      );
      let interval = fromVC.withdrawnAt
        .minus(fromVC.depositedAt)
        .toBigDecimal();

      apy = BigInt.fromU32(86400 * 365)
        .toBigDecimal()
        .div(interval)
        .times(ln);
    } else {
    ...
    updateVaultControllerDailyStat(
      fromVC,
      apy,
      toVT.depositedPercentage,
      event.block.timestamp,
    );

    toVT.apy = apy.times(
      dp.value.toBigDecimal().div(BigDecimal.fromString('10000')),
    );
