...
export function updateSushiAssetsBasePrice(
  reserve0: BigInt,
  reserve1: BigInt,
  asset0: Asset,
  asset1: Asset,
  ...
  if ([BNA_ADDRESS].includes(asset0.id)) {
    updateSushiAssetBasePrice(asset0, asset1, asset0Reserve, asset1Reserve);
  }

  if ([BNA_ADDRESS].includes(asset1.id)) {
    updateSushiAssetBasePrice(asset1, asset0, asset1Reserve, asset0Reserve);
  }
}
...
function updateSushiAssetBasePrice(
  ...
  asset.basePriceSushi = baseAssetReserve
    .div(assetReserve)
    .times(baseAsset.basePriceSushi);
  asset.save();
  /*
        p = Q_base / Q_quote
